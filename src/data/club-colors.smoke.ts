...
const crestFiles = readdirSync(crestDir).filter((f) => f.endsWith('.webp'))

for (const id of ids) {
  assert(crestFiles.includes(`${id}.webp`), `${id} has a crest at src/assets/crests/${id}.webp`)
}
for (const file of crestFiles) {
  assert(Boolean(clubs[file.replace(/\.webp$/, '')]), `crest ${file} matches a registry slug`)
}

// Crests are generated by scripts/fetch-crests.ts, so the shape guarantees
// (square, trimmed, alpha) come from the encoder. What the repo still has to
// pin is that these are the encoder's output and not something pasted in by
// hand: a real WebP container, and small enough that nobody has dropped a
// full-size master into a directory that is preloaded on every page load.
const CREST_SIZE_CEILING = 24 * 1024
for (const file of crestFiles) {
  const bytes = readFileSync(new URL(file, crestDir))
  assert(
    bytes.subarray(0, 4).toString('ascii') === 'RIFF' &&
      bytes.subarray(8, 12).toString('ascii') === 'WEBP',
    `${file} is a real WebP container`,
  )
  assert(
    bytes.length <= CREST_SIZE_CEILING,
    `${file} is ${(bytes.length / 1024).toFixed(1)} KB, over the ${CREST_SIZE_CEILING / 1024} KB crest ceiling`,
  )
}
