// Generated — do not hand-edit
import type { HighlightVideo } from '../types'

/**
 * Auto-curated ESPN FC, ESPN Deportes highlight cuts for esp1 2026-27, keyed by match id.
 *
 * GENERATED by scripts/curate-club-videos.ts — do not edit by hand. Entries are
 * appended as the rights holder publishes a full-match cut that clears every
 * one of the curator's deterministic gates; nothing is ever edited or removed.
 *
 * No entry carries `durationSeconds`, and none ever should: that omission is
 * what keeps a runtime badge off club matches.
 */
export const esp1_2026_videos: Record<string, HighlightVideo[]> = {
  'esp1-alaves-valencia': [{ youtubeId: 'MxVq5adjkVU', kind: 'normal' }],
  'esp1-alaves-villarreal': [{ youtubeId: 'Z48lJUXIG20', kind: 'normal' }],
  'esp1-athletic-club-alaves': [{ youtubeId: 'jjDZU_NpYIQ', kind: 'normal' }],
  'esp1-athletic-club-atletico-madrid': [{ youtubeId: 'P9prR9H28Gg', kind: 'normal' }],
  'esp1-athletic-club-elche': [{ youtubeId: '6FyomrQosP8', kind: 'normal' }],
  'esp1-athletic-club-sevilla': [{ youtubeId: 'Q-kKpIvXp0k', kind: 'normal' }],
  'esp1-atletico-madrid-osasuna': [{ youtubeId: 'vIYc6FMKy8Y', kind: 'normal' }],
  'esp1-atletico-madrid-real-madrid': [{ youtubeId: 'bLzrK9H8XLc', kind: 'normal' }],
  'esp1-atletico-madrid-villarreal': [{ youtubeId: 'aEleu6mtpxk', kind: 'normal' }],
  'esp1-barcelona-athletic-club': [{ youtubeId: '0OHaoF7myVY', kind: 'normal' }],
  'esp1-barcelona-racing-santander': [{ youtubeId: 'bWC2XazMFK8', kind: 'normal' }],
  'esp1-barcelona-rayo-vallecano': [{ youtubeId: 'KIA683_HiLw', kind: 'normal' }],
  'esp1-celta-vigo-athletic-club': [{ youtubeId: 'H8gfjh4f0iE', kind: 'normal' }],
  'esp1-celta-vigo-racing-santander': [{ youtubeId: '9gwBByzkNZE', kind: 'normal', publisher: 'espn-deportes' }],
  'esp1-deportivo-real-betis': [{ youtubeId: 'UD3hrcbpcWM', kind: 'normal' }],
  'esp1-deportivo-valencia': [{ youtubeId: 'aUYYjd9lAKs', kind: 'normal' }],
  'esp1-elche-barcelona': [{ youtubeId: 'T1B9zsH9adY', kind: 'normal' }],
  'esp1-elche-real-madrid': [{ youtubeId: '3EhCO20HrB4', kind: 'normal' }],
  'esp1-elche-real-sociedad': [{ youtubeId: '4Sgi6wmKi88', kind: 'normal' }],
  'esp1-espanyol-elche': [{ youtubeId: '_WLWxOTY2Ck', kind: 'normal' }],
  'esp1-espanyol-real-madrid': [{ youtubeId: 'i1rmAdOSfuA', kind: 'normal' }],
  'esp1-espanyol-sevilla': [{ youtubeId: 'Rvz6erOp-k0', kind: 'normal' }],
  'esp1-getafe-malaga': [{ youtubeId: 'VTqhYR74sHY', kind: 'normal', publisher: 'espn-deportes' }],
  'esp1-levante-barcelona': [{ youtubeId: 'bSeIHtsdJyo', kind: 'normal' }],
  'esp1-levante-real-betis': [{ youtubeId: 'JIaT0ZrCXV8', kind: 'normal' }],
  'esp1-osasuna-getafe': [{ youtubeId: 'wmCXSxhPJDg', kind: 'normal' }],
  'esp1-osasuna-rayo-vallecano': [{ youtubeId: 'dDF68xJW-Mk', kind: 'normal', publisher: 'espn-deportes' }],
  'esp1-real-betis-getafe': [{ youtubeId: 'QAIYVBuuh0c', kind: 'normal' }],
  'esp1-real-betis-real-madrid': [{ youtubeId: 'Ls7lgDXcZp8', kind: 'normal' }],
  'esp1-real-betis-real-sociedad': [{ youtubeId: 'pJRJsX-3U4w', kind: 'normal' }],
  'esp1-real-madrid-malaga': [{ youtubeId: 'BSb9iSg0D4s', kind: 'normal' }],
  'esp1-real-madrid-rayo-vallecano': [{ youtubeId: 'GtscnZUGLe4', kind: 'normal' }],
  'esp1-real-madrid-real-sociedad': [{ youtubeId: 'K_4yDlQknUI', kind: 'normal' }],
  'esp1-real-sociedad-atletico-madrid': [{ youtubeId: '7Rb78W9GlM4', kind: 'normal' }],
  'esp1-real-sociedad-celta-vigo': [{ youtubeId: 'w6vspBHbyr0', kind: 'normal' }],
  'esp1-real-sociedad-espanyol': [{ youtubeId: 'tgYYdJCBFY4', kind: 'normal' }],
  'esp1-sevilla-atletico-madrid': [{ youtubeId: 'PkBJJjyfV_A', kind: 'normal' }],
  'esp1-sevilla-barcelona': [{ youtubeId: 'ujkBRg-upZY', kind: 'normal' }],
  'esp1-sevilla-valencia': [{ youtubeId: 'l4qqAX2GS7o', kind: 'normal' }],
  'esp1-valencia-barcelona': [{ youtubeId: 'BVeuyol-th4', kind: 'normal' }],
  'esp1-valencia-real-betis': [{ youtubeId: 'qcxVr9sggWw', kind: 'normal' }],
  'esp1-valencia-real-sociedad': [{ youtubeId: '55MzS3qrRJ8', kind: 'normal' }],
  'esp1-villarreal-levante': [{ youtubeId: 'LAR13_KP79g', kind: 'normal', publisher: 'espn-deportes' }],
  'esp1-villarreal-real-betis': [{ youtubeId: 'GrjB1mwr6GQ', kind: 'normal' }],
}
